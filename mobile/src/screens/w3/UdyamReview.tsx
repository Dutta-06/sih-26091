...
export function UdyamReview({ fields, answers }: { fields: Field[]; answers: Record<string, string> }) {
  ...
  return (
    <div className="overflow-hidden rounded-[var(--radius-card)] bg-white shadow-[var(--shadow-card)]">
      <div className="border-b-4 border-marigold-500 bg-azure-800 px-4 py-3 text-white">
        <p className="text-[11px] tracking-wide text-azure-100 uppercase">{t("udyam.form.ministry")}</p>
            ...
            <ul className="mt-1 divide-y divide-dashed divide-line">
              {rows.map(([k, v]) => (
                <li key={k} className="flex items-start gap-2.5 py-2">
                  <CheckCircle2 className="mt-0.5 size-4 shrink-0 text-azure-600" />
...
export function UdyamSuccess({ fresh, number }: { fresh: boolean; number: string | null }) {
  const { t } = useI18n();
  return (
    <Card tone="azure" className="relative overflow-hidden text-center">
      <motion.div initial={fresh ? { scale: 0.4, rotate: -20 } : false} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", stiffness: 260, damping: 14 }} className="mx-auto grid size-16 place-items-center rounded-full bg-marigold-500 text-azure-950">
        <BadgeCheck className="size-9" />
      </motion.div>
      <p className="mt-3 text-[20px] font-bold">{t(fresh ? "udyam.success.title" : "udyam.success.already")}</p>
      <p className="mt-1 text-[13px] text-azure-100">{t("udyam.success.numberLabel")}</p>
      {number ? (
        <button
        ...
        >
          {number}
          <Copy className="size-4 shrink-0 text-azure-100" />
        </button>
      ) : (
        <p className="mt-1 text-[15px] font-semibold">{t("g3.udyam.numberElsewhere")}</p>
      )}
      <p className="mt-3 text-[13px] leading-snug text-azure-100">{t("g3.udyam.successBody")}</p>
    </Card>
  );
}
