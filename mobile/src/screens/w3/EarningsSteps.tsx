    ...
    <Reveal i={i}>
      <Card>
        <div className="flex items-start gap-3">
          <span className="tabular grid size-8 shrink-0 place-items-center rounded-full bg-forest-100 text-sm font-bold text-forest-800">{n}</span>
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <p className="text-[15px] leading-snug font-semibold">{title}</p>
              <ConfidenceBadge value={confidence} />
            </div>
            <p className="tabular mt-1 text-[22px] leading-tight font-bold text-forest-800">{value}</p>
  ...
  return (
    <div className="mt-3">
        ...
        {values.map((v, i) => {
          ...
          return (
            <div key={i} className="flex h-full flex-1 flex-col items-center justify-end">
              <div className={cx("w-full rounded-t-md", isLow ? "bg-clay-600" : "bg-forest-600")} style={{ height: (v / max) * H }} />
            </div>
          );
        })}
      </div>
      <div className="mt-1 flex gap-1 text-center text-[10px] text-ink-3">
        {values.map((_, i) => (
          <span key={i} className={cx("flex-1", low.includes(i) && "font-semibold text-clay-700")}>
            {t(`earn.month.${i}`)}
          </span>
        ))}
      </div>
