...
import { Badge, Card, cx, IconBubble, Reveal, Ring, Section, TabScreen } from "../ui";
...
export default function Home() {
  ...
  const monthLabel = (ym: string) => new Date(`${ym}-01T00:00:00Z`).toLocaleDateString(LANG_INFO[lang].dateLocale, { month: "long", year: "numeric", numberingSystem: "latn" });

  const tiles = [
    { icon: Calculator, key: "calculator", tone: "azure" as const, go: () => open("plan") },
    ...
    { icon: BriefcaseBusiness, key: "business", tone: "azure" as const, go: () => open("business") },
  ];
  ...
  return (
    ...
    >
      ...
      <Reveal i={0}>
        <div className="relative mt-3 overflow-hidden rounded-[var(--radius-card)] bg-azure-800 p-4 text-white shadow-[var(--shadow-float)]">
          <div aria-hidden className="absolute -top-16 -right-12 size-44 rounded-full bg-azure-700/60" />
          <div className="relative">
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs font-semibold tracking-wide text-azure-100 uppercase">{t("home.journey")}</p>
              <span className="tabular text-xs text-azure-100">{t("home.stepOf", { n: step.index + 1, total: STAGES.length })}</span>
            </div>
            <h2 className="mt-1 text-xl font-bold">{t(`home.stage.${step.stage}`)}</h2>
            <p className="mt-1 text-[13px] leading-snug text-azure-100">{t(`home.stageHint.${step.cta}`)}</p>
                ...
                <div key={s} className={cx("flex items-center", i < STAGES.length - 1 && "flex-1")}>
                  <motion.span
                    ...
                    className={cx(
                      ...
                      i < step.index ? "bg-marigold-500 text-azure-950" : i === step.index ? "bg-white text-azure-800 ring-4 ring-white/25" : "bg-white/15 text-azure-100",
                    )}
                  >
            ...
            <div className="mt-1.5 flex justify-between text-[10px] text-azure-100">
            ...
            <motion.button
              ...
              className={cx(
                ...
                step.tone === "clay" ? "bg-clay-600 text-white active:bg-clay-700" : "bg-marigold-500 text-azure-950 active:bg-marigold-600",
              )}
            >
              {t(`home.cta.${step.cta}`)}
      ...
      <Reveal i={1}>
        <Card className="mt-4 flex items-center gap-4" onClick={() => open("assistant")}>
          <span className="relative grid size-14 shrink-0 place-items-center rounded-full bg-azure-700 text-white">
            <motion.span aria-hidden className="absolute inset-0 rounded-full bg-azure-600" animate={{ scale: [1, 1.35], opacity: [0.5, 0] }} transition={{ duration: 1.6, repeat: Infinity }} />
          ...
          <Section title={t("home.health")}>
            <Card className="flex items-center gap-4" onClick={() => open("home", { name: health.earlyWarning ? "warning" : "monitoring" })}>
              <Ring
                ...
                size={72}
                stroke={8}
                tone={health.band === "healthy" ? "azure" : health.band === "at_risk" ? "clay" : "marigold"}
              />
              <div className="min-w-0 flex-1">
                {health.band ? (
